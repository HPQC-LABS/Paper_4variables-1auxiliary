...
b1=b(:,1);b2=b(:,2);b3=b(:,3);b4=b(:,3);b5=b(:,5);b6=b(:,6);b7=b(:,7);b8=b(:,8);ba1=b(:,9);ba2=b(:,10);

blue_LHS=b1.*b2.*b3.*b4 + b1.*b2.*b3 + b1.*b2.*b4 + 2*b1.*b3.*b4 + 3*b2.*b3.*b4;
red_LHS= (-1)*b5.*b6.*b7.*b8 - 2*b5.*b6.*b7 - 3*b5.*b6.*b8 - 4*b5.*b7.*b8 - 5*b6.*b7.*b8;
blue_RHS=3*b1.*b2 + 4*b1.*b3 + 4*b1.*b4 + 5*b2.*b3 + 5*b2.*b4 + 6*b3.*b4 + ba1.*(10 - 6*b1 - 7*b2 - 8*b3 - 8*b4);
red_RHS= (-1)*ba2.*(31 - 10*b5 - 11*b6 - 12*b7 - 13*b8);
LHS = blue_LHS + red_LHS + b1.*b8;
RHS = blue_RHS + red_RHS + b1.*b8;

% Pg. 6, Eqs 23-28
b= dec2bin(2^8-1:-1:0)-'0';
b1=b(:,1);b2=b(:,2);b3=b(:,3);b4=b(:,3);b5=b(:,5);ba1=b(:,6);ba2=b(:,7);ba3=b(:,8);
...
% Pg. 6-7, Eqs 32-38
b= dec2bin(2^10-1:-1:0)-'0';
b1=b(:,1);b2=b(:,2);b3=b(:,3);b4=b(:,3);b5=b(:,5);ba1=b(:,6);ba2=b(:,7);ba3=b(:,8);ba4=b(:,9);ba5=b(:,10);
blue_LHS = 5*b1.*b2.*b3.*b4 - 3*b1.*b2.*b3 - b1.*b2.*b4 - b1.*b3.*b4 - 2*b2.*b3.*b4;
blue_RHS = b1.*b2 + b1.*b3 + 3*b1.*b4 + 2*b2.*b4 + 2*b3.*b4 - ba1.*(5*b1 + 4*b2 + 4*b3 + 6*b4 - 8);
...
b1=b(:,1);b2=b(:,2);b3=b(:,3);b4=b(:,3);ba=b(:,5);
LHS = atan(b1 + b2).*exp(min(b2,b3)).*sqrt(5*b4);
RHS = -5.70 + 0.20*b1.*b2 + 1.24*b1.*b3 + 1.96*b1.*b4 + 4.26*b2.*b3 + 4.98*b2.*b4 + 4.26*b3.*b4 - 1.44*b1.*ba + 4.46*b2.*ba + 5.50*b3.*ba + 4.46*b4.*ba;
